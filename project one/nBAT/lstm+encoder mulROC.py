...
from lstmmul import model3
from tfdpbioyuYft import predicted4,true_intent2
...
train, val, test = data.TabularDataset.splits(
    ...
    validation='biosyu3L.test.csv',
    test='biosyu3L.valid.csv',
    format='csv',
...
model2 = AttentionLSTM(vocab_size,input_size, hidden_size, layer, num_class,
                      n_layers,
                      n_heads,  # 多头self-attention
                      hid_dim,
                      dropout,
...
def evaluate(model, model2, model3,iterator, loss_intent,predicted4,true_intent2):
    model.eval()
    ...
    with torch.no_grad():
        for i, batch in enumerate(iterator):
            ...
            intent_output, kl = model2(src.long(), yu.float(),
                                                    tag=0)  # [batch_size, intent_dim]; [batch_size, trg_len-1, slot_size]
            ...
            intent_output, kl = model(src.long(),
                                                   yu.float(),
                                                   tag=1)  # [batch_size, intent_dim]; [batch_size, trg_len-1, slot_size]
            # intent_output = Fn.softmax(intent_output, dim=-1)
            # _, output2 = torch.max(intent_output, 1)
            output3 = intent_output[:, 1]
            output3 = output3.cpu()
            ...
            intent_output = model3(src.long(),
                                                   yu.float())  # [batch_size, intent_dim]; [batch_size, trg_len-1, slot_size]
            # intent_output = Fn.softmax(intent_output, dim=-1)
            # _, output2 = torch.max(intent_output, 1)
            output3 = intent_output[:, 1]
            output3 = output3.cpu()
        ...
        auc4 = roc_auc_score(y_test, y_pred_4)

        plt.figure(figsize=(8, 5))
        lw = 2
        plt.plot(fpr1, tpr1, color='darkorange', markersize=4,
                 lw=lw, label='nBAT (AUC = %0.4f)' % auc1)
        plt.plot(fpr2, tpr2, color='darkgreen', markersize=4,
                 lw=lw, label='nBAT(no feature) (AUC = %0.4f)' % auc2)
        plt.plot(fpr4, tpr4, color='darkred', markersize=4,
                 lw=lw, label='sequence+Feature+encoder (AUC = %0.4f)' % auc4)
        plt.plot(fpr3, tpr3, color='darkblue', markersize=4,
                 lw=lw, label='sequence+Feature+BiLSTM+Attention (AUC = %0.4f)' % auc3)
...
def predictroc(iterator, loss_intent,count,count2,count3,model,model2,model3,predicted4,true_intent2):
    bios_data = os.path.join(base_dir, 'model')
    saved_model_path1=os.path.join(bios_data, "modeltF4-{0}.pth".format(count))
    saved_model_path2 = os.path.join(bios_data, "modelt4-{0}.pth".format(count2))
    saved_model_path3 = os.path.join(bios_data, "modelt3-{0}.pth".format(count3))
    ...
    evaluate(model,model2,model3, iterator, loss_intent,predicted4,true_intent2)
...
if zhiling=='1':
    ...
    predictroc(test_iter, loss_intent,count,count2,count3,model,model2,model3,predicted4,true_intent2)
