...
def train(model, iterator, optimizer, loss_intent, clip):
    ...
    for i, batch in enumerate(iterator):
        ...
        intent_output, kl = model(yu.long(), yu)  # [batch_size, intent_dim]; [batch_size, trg_len-1, slot_size]
...
def predict(model, iterator, loss_intent, count):
    ...
    saved_model_path = os.path.join(bios_data, "modeltof4-{0}.pth".format(count))
...
if zhiling == '1':
    ...
    count = input("序号：")
    saved_model_path = os.path.join(os.getcwd(), "modelt4-{0}.pth".format(count))
    model.load_state_dict(torch.load(saved_model_path))
    # model = torch.load(saved_model_path).to(DEVICE)
    for epoch in range(n_epochs):
        ...
        if eva_score0 > best_score:
            ...
            model_path = os.path.join(os.getcwd(), "modelt4-{0}.pth".format(epoch))
            model_path2 = os.path.join(os.getcwd(), "modelt4-{0}.pth".format(epoch - er))
